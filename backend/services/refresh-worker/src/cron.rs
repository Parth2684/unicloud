...
pub async fn refresh_quota() -> Result<(), String>{
    ...
    let refreshed_quota = match QuotaEntity::update_many()
        ...
        .col_expr(QuotaColumn::RefreshAt, Expr::cust("(CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::date + 30"))
        .filter(QuotaColumn::RefreshAt.eq(date))
        .exec_with_returning(db)
        .await {
            Err(err) => {
                eprintln!("Error running cron jon as data retrieval from db failed: {:?}", err);
