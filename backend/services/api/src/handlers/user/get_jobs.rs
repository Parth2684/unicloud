...
use entities::{
    ...
    job::{self, Column as JobColumn, Entity as JobEntity, Relation as JobRelation},
    sea_orm_active_enums::{LinkType, Provider, Status, TransferType},
};
use sea_orm::{
    ColumnTrait, ConnectionTrait, DbBackend, EntityTrait, FromQueryResult, QueryFilter, QueryResult, QuerySelect, RelationTrait, Statement, prelude::Expr, sea_query::Alias
};
use serde::Serialize;
use serde_json::json;
use uuid::Uuid;
...
pub async fn get_jobs(Extension(claims): Extension<Claims>) -> Result<Response, AppError> {
    ...
    let jobs = JobEntity::find()
        ...
        .join_as(sea_orm::JoinType::LeftJoin, JobRelation::CloudAccount1.def(), Alias::new("source_account"))
        .join_as(sea_orm::JoinType::LeftJoin, JobRelation::CloudAccount2.def(), Alias::new("destination_account"))
        .select_only()
           .column(JobColumn::Id)
