...
use entities::{
    cloud_account::{Column as CloudColumn, Entity as CloudEntity},
...
pub async fn delete_file_google(
    Extension(claims): Extension<Claims>,
    Path((drive_id, file_id)): Path<(Uuid, String)>,
    ...
    if let Ok(Some(acc)) = cloud_account {
        match decrypt(&acc.access_token) {
            Err(_) => Err(AppError::Forbidden(Some(String::from(
                "Error decrypting access token please try refreshing the account",
            )))),
            Ok(token) => {
